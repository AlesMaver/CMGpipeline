...
workflow Conifer {
  ...
  # Always run MakeRPKM to generate RPKM file
  call MakeRPKM {
      input:
  ...
  # Only run the full Conifer analysis if reference RPKMs are provided
  if (defined(input_reference_rpkms)) {
    call CONIFER_Analyze {
        input:
          input_rpkm=MakeRPKM.output_rpkm,
          input_reference_rpkms=select_first([input_reference_rpkms]),
  ...
  output {
    # Always output RPKM file
    File output_rpkm = MakeRPKM.output_rpkm
    
    # Optional outputs - only available when reference RPKMs are provided
